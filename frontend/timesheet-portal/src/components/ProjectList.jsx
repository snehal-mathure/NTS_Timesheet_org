...
export default function ProjectList() {
  ...
  // When projects or pageSize change, ensure current page is valid
  useEffect(() => {
    const totalPages = Math.max(1, Math.ceil(projects.length / pageSize));
    if (page > totalPages) setPage(totalPages);
    // no eslint disable needed here
  ...
  const loadData = async () => {
    try {
      ...
      // reset to first page after load/search/filter change
      setPage(1);
    } catch (err) {
  ...
  // ❗ NEW: compute displayedProjects for current page
        ...
        <div className="max-w-5xl w-full mx-auto mt-4 md:mt-6 space-y-5">
          ...
          <div className="bg-white/90 border border-[#e5e7f5] rounded-3xl shadow-[0_24px_60px_rgba(15,23,42,0.12)] overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#e5e7f5] bg-white/80">
              ...
              <Link
                to="/addproject"
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl font-semibold text-white text-sm"
                style={{
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-2xl 
                            border border-slate-200 bg-white text-xs font-medium text-slate-700 
                            hover:bg-slate-50 transition"
                >
                  <FiFilter className="w-3.5 h-3.5 text-slate-700" />
                  Reset
                </button>
              </div>
            ...
            <div className="fixed inset-0 z-40 flex items-center justify-center">
              ...
              <div className="relative bg-white rounded-3xl shadow-xl w-[90%] max-w-md p-5">
                ...
                <div className="flex justify-end gap-2 mt-5">
                  ...
                  <button className="inline-flex items-center px-4 py-2 rounded-2xl text-xs font-medium border border-teal-300 bg-teal-100 text-teal-700 hover:bg-teal-200 hover:border-teal-400 hover:text-teal-900 transition" onClick={handleEditSubmit}>
                    Save Changes
                  </button>
                </div>
              </div>
