...
//  *  - overrideIsAdmin (optional) - if provided, overrides localStorage is_admin check (useful for tests)
//  *
//  * NOTE: kept original logic (isAdmin check, logout behavior, nav items) but formatted to match
//  * the AdminSidebar structure (mobile top bar, fixed desktop aside, collapse behavior, footer).
//  */
// export default function UserDashboardSidebar({ overrideIsAdmin = null }) {
//   // read from localStorage lazily to avoid potential SSR issues
//   const [collapsed, setCollapsed] = useState(() => {
...
//   // Using sizes similar to AdminSidebar but keeping component logic intact
//   const baseItem =
//     "flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150";
...
//   return (
//     <>
//       {/* MOBILE TOP BAR (matches AdminSidebar style) */}
...
//             {mobileOpen ? <FiChevronLeft size={20} /> : <FiMenu size={20} />}
//           </button>
//           <span className="font-semibold text-slate-800 text-[13px]">Timesheet App</span>
//         </div>
...
//           } transition-all duration-200 bg-white rounded-r-3xl shadow-[0_24px_60px_rgba(15,23,42,0.12)] border-r border-[#e5e7f5] overflow-hidden`}
//         >
//           {/* HEADER (AdminSidebar-like) */}
//           <div className="px-3 py-3 flex items-center justify-between">
//             <div className={`flex items-center gap-2 ${collapsed ? "justify-center w-full" : ""}`}>
//               <div className="w-8 h-8 rounded-xl bg-[#f0f3ff] flex items-center justify-center">
//                 <img src={logo2} className="w-5 h-5 object-contain rounded" alt="logo" />
//               </div>

//               {!collapsed && (
//                 <div>
//                   <div className="text-[11px] font-semibold text-slate-900">Timesheet App</div>
...
//           {/* NAVIGATION (matches Admin layout & keeps original nav logic) */}
//           <nav className="px-2 pb-3 pt-1 flex-1 overflow-auto" aria-label="Main navigation">
//             <ul className="space-y-1">
//               {navItems.map(({ to, label, Icon }) => (
//                 <li key={to}>
...
//                     <div className="text-[11px] font-semibold text-slate-800">{fname} {lname}</div>
//                   </div>
//                 )}
//               </div>
...
//               <button
//                 onClick={logout}
//                 className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-medium border border-[#e0e4ff] hover:bg-[#f3f5ff] ${collapsed ? "mx-auto mt-2" : ""}`}
//               >
...
//       {/* MOBILE OVERLAY */}
//       {mobileOpen && <button onClick={() => setMobileOpen(false)} className="fixed inset-0 z-30 bg-black/30 md:hidden" aria-label="Close sidebar" />}
//     </>
//   );
// }
...
const isAdmin =
  ...
  const navItems = [
    ...
    { to: `/editemployee/${empId}`, label: "Edit Profile", Icon: FiEdit },
